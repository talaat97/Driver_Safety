...
class HomeCubit extends Cubit<HomeStates> {
  ...
  void getUserFromCloud() async
  {
    if(FirebaseAuth.instance.currentUser !=null)
    {
      ...
      if(contactResponse1.exists){
        userModel!.firstContactModel = ContactModel.fromJson(contactResponse1.data()!);
        var contract1 = await FirebaseFirestore.instance.collection('users')
            .where('phone', isEqualTo: userModel!.firstContactModel!.phone!).get();

        userModel!.firstContactModel!.id = contract1.docs.first.id;

      }
      if(contactResponse2.exists){
        userModel!.secondContactModel = ContactModel.fromJson(contactResponse2.data()!);
        var contract2 = await FirebaseFirestore.instance.collection('users')
            .where('phone', isEqualTo: userModel!.secondContactModel!.phone!).get();
        userModel!.secondContactModel!.id = contract2.docs.first.id;
      }
